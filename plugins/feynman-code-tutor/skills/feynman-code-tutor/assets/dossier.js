...
(function () {
  ...
  function renderReport() {
    ...
    function row(name, st) {
      rows.push('<div class="rrow"><span>' + name + '</span><span class="rstat ' + st + '">'
        + label(st) + "</span></div>");
    }
    row(S.lang === "es" ? "La hoja en blanco" : "The blank page", statusOf("blank"));
    CFG.conceptIds.forEach(function (cid) {
      var nm = CFG.names[cid] || {};
      row(nm[S.lang] || nm.en || cid, conceptStatus(cid));
    });
    row(S.lang === "es" ? "Reconstrucción" : "Rebuild", statusOf("rebuild"));
    row(S.lang === "es" ? "Registro senior" : "Senior register", statusOf("senior"));
    var el = $("#reportbody");
  ...
  function reportText() {
    ...
    $$("#reportbody .rrow").forEach(function (r) {
      out.push("- " + r.children[0].textContent + ": " + r.children[1].textContent);
    });
